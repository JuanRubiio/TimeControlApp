...
export const configurationApi={
  ...
  createEmployment:(input:{employeeId:string;siteId:string;managerEmployeeId:string|null;effectiveFrom:string;effectiveTo:string|null})=>create<Employment>('/api/v1/employments',input), createZone:(input:{label:string;latitude:number;longitude:number;radiusMeters:number})=>create<WorkLocationZone>('/api/v1/work-location-zones',input), deleteZone:(id:string)=>request<WorkLocationZone>(`/api/v1/work-location-zones/${id}`,{method:'DELETE'}), createClockingPolicy:(input:{employmentId:string;method:'web'|'geo_punctual';locationZoneId:string|null;reasonCode:'fixed_site'|'approved_mobility';effectiveFrom:string;effectiveTo:string|null})=>create<ClockingPolicy>('/api/v1/clocking-policies',input),
