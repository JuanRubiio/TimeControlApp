...
export function RealZoneMap({ latitude, longitude, radiusMeters, onPointChange }: Point & { radiusMeters: number; onPointChange: (point: Point) => void }) {
  ...
  useEffect(() => {
    ...
    void import('leaflet').then((L) => {
      ...
      const marker = L.marker(initialPoint, { draggable: true, icon: L.divIcon({ className: 'zone-map-pin', html: '<span aria-hidden="true"></span>', iconSize: [20, 20], iconAnchor: [10, 10] }), title: 'Centro de la zona autorizada' }).addTo(map);
