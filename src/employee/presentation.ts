...
/** El estado actual viene de la proyección autorizada del servidor; no del último evento histórico. */
export function actionsForCurrentStatus(status:'not_started'|'working'|'on_break'|'ended'|undefined,last?:TimeEvent):TimeEventType[]{if(!status)return availableActions(last);if(status==='not_started'||status==='ended')return ['clock_in'];if(status==='on_break')return ['break_end'];return ['break_start','clock_out'];}
