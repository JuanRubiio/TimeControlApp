...
export async function passwordLogin(email:string, password:string, correlationId:string) {
  ...
  const destination = defaultDestination(user);
  if (isLocalSyntheticDemoMfaBypassEnabled({enabled:config.LOCAL_SYNTHETIC_DEMO_MFA_BYPASS,sessionCookieSecure:config.SESSION_COOKIE_SECURE,email:user.email,roles:user.roles})) { await appendAudit({actorType:'user',actorId:user.id,action:'auth.mfa.bypassed_synthetic_demo',resourceType:'session',result:'success',correlationId,changes:{localSyntheticDemo:true}}); return { status:'authenticated' as const, session: await createSession(user.id, correlationId), destination }; }
  if (user.roles.includes('admin') && !user.mfa_enrolled_at) return { status: 'mfa_enrollment_required' as const, challenge: await createChallenge(user.id, 'mfa_enroll'), destination };
  if (user.roles.includes('admin')) return { status: 'mfa_required' as const, challenge: await createChallenge(user.id, 'mfa_verify'), destination };
  return { status:'authenticated' as const, session: await createSession(user.id, correlationId), destination };
}
export async function finishMfa(userId:string, code:string, correlationId:string) {
  const result = await db.query<{mfa_secret:string|null;roles:string[]}>(`SELECT u.mfa_secret, coalesce(array_agg(DISTINCT r.code) FILTER (WHERE r.code IS NOT NULL), '{}') roles FROM users u LEFT JOIN user_role_assignments ura ON ura.user_id=u.id AND ura.valid_from<=clock_timestamp() AND (ura.valid_to IS NULL OR ura.valid_to>clock_timestamp()) LEFT JOIN roles r ON r.id=ura.role_id WHERE u.id=$1 AND u.is_active=true GROUP BY u.id`, [userId]); const user=result.rows[0]; const secret=user?.mfa_secret;
  ...
  return { session: await createSession(userId, correlationId), destination: defaultDestination(user) };
}
...
export async function currentActor(token:string): Promise<ScopedActor|null> { const q=await db.query<{id:string;roles:string[];permissions:string[];scopes:{type:'self'|'site'|'environment';id:string|null}[]}>(`SELECT u.id,coalesce(array_agg(DISTINCT r.code) FILTER (WHERE r.code IS NOT NULL),'{}') roles,coalesce(array_agg(DISTINCT rp.permission_code) FILTER (WHERE rp.permission_code IS NOT NULL),'{}') permissions,coalesce(jsonb_agg(DISTINCT jsonb_build_object('type',ura.scope_type,'id',ura.scope_id)) FILTER (WHERE ura.id IS NOT NULL),'[]'::jsonb) scopes FROM auth_sessions s JOIN users u ON u.id=s.user_id LEFT JOIN user_role_assignments ura ON ura.user_id=u.id AND ura.valid_from<=clock_timestamp() AND (ura.valid_to IS NULL OR ura.valid_to>clock_timestamp()) LEFT JOIN roles r ON r.id=ura.role_id LEFT JOIN role_permissions rp ON rp.role_id=r.id WHERE s.token_hash=$1 AND s.revoked_at IS NULL AND s.expires_at>clock_timestamp() AND u.is_active=true GROUP BY u.id`,[tokenHash(token)]); const row=q.rows[0]; return row ? {id:row.id,roles:row.roles,permissions:new Set(row.permissions),scopes:row.scopes}:null; }
