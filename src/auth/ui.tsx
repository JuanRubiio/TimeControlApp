...
export default function Login(){
  const router=useRouter(); const params=useSearchParams(); const destination=safeReturnTo(params.get('returnTo'))??'/employee'; const [email,setEmail]=useState(''); const [password,setPassword]=useState(''); const [code,setCode]=useState(''); const [mfa,setMfa]=useState(false); const [enrollment,setEnrollment]=useState(false); const [uri,setUri]=useState(''); const [error,setError]=useState(''); const [busy,setBusy]=useState(false);
  const complete=()=>{router.replace(destination);router.refresh();};
  const submit=async(event:FormEvent)=>{event.preventDefault();setError('');setBusy(true);try{const result=await api('/api/v1/auth/login',{email,password});if(result?.mfaRequired){setMfa(true);setEnrollment(!!result.enrollmentRequired);if(result.enrollmentRequired){const setup=await api('/api/v1/auth/mfa/setup');setUri((setup as {otpauthUri?:string})?.otpauthUri??'');}}else{complete();}}catch(value){setError(value instanceof Error?value.message:'No se pudo iniciar sesión.');}finally{setBusy(false);}};
  const verify=async(event:FormEvent)=>{event.preventDefault();setError('');setBusy(true);try{await api('/api/v1/auth/mfa/verify',{code});complete();}catch(value){setError(value instanceof Error?value.message:'No se pudo verificar el código.');}finally{setBusy(false);}};
