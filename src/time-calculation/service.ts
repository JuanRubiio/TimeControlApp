...
export async function recalculateDaily(employeeId:string,laborDate:string,actorId:string|undefined,correlationId:string):Promise<{calculation:PersistedDailyCalculation;replayed:boolean}>{
  const client=await db.connect(); try{await client.query('BEGIN'); await client.query('SELECT pg_advisory_xact_lock(hashtextextended($1, 505))',[`${employeeId}:${laborDate}`]);
    ...
    const result=saved({...version,employeeId,laborDate,siteId:calculation.siteId}); await appendAudit({actorType:actorId?'user':'system',actorId,action:'time-calculation.recalculated',resourceType:'daily-calculation',resourceId:daily.id,result:'success',correlationId,changes:{employeeId,laborDate,ruleVersionId:rule.ruleVersionId,revision:version.revision,incidentCodes:calculation.incidents.map(i=>i.code)}});
