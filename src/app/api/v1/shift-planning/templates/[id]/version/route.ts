...
import { createTemplateVersion } from '@/shift-planning/service';
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!parsed.success) return invalid(auth.cid);
  try {
    const { id } = await params;
    return data(await createTemplateVersion(id, parsed.data, { actor: auth.actor, correlationId: auth.cid }), auth.cid, 201);
  } catch (value) {
