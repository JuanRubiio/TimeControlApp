...
const input=z.object({sourceId:z.string().min(1),locality:z.string().trim().min(1).max(120),year:z.number().int().min(2020).max(2100)});
export async function POST(request:NextRequest){const access=await authenticated(request,'shift-planning.write');if('response'in access)return access.response;const parsed=input.safeParse(await request.json().catch(()=>null));if(!parsed.success)return invalid(access.cid);try{return data(await previewOfficialHolidays(parsed.data.sourceId,parsed.data.locality,parsed.data.year),access.cid);}catch(error){const code=error instanceof Error?error.message:'';const message=code==='HOLIDAY_SOURCE_NOT_READY'?'Esta fuente está catalogada, pero todavía no dispone de adaptador validado.':'No se pudo consultar la fuente oficial ahora mismo.';return NextResponse.json({error:{code:code||'HOLIDAY_SOURCE_UNAVAILABLE',message},correlationId:access.cid},{status:code==='HOLIDAY_SOURCE_NOT_FOUND'?404:503});}}
