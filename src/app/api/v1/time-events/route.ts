...
import { processCalculationOutbox } from '@/time-calculation/outbox';
...
export async function POST(request:NextRequest) {const cid=correlationId();const access=await actorFor(request,'time-event.create:self',cid);if('response'in access)return access.response;const command=timeEventCommand.safeParse(await request.json().catch(()=>null));if(!command.success)return error('VALIDATION_FAILED','La solicitud de fichaje no es válida.',422,cid);const employeeId=await employeeForUser(access.actor.id);if(!employeeId)return error('FORBIDDEN','No existe una ficha de empleado activa para fichar.',403,cid);try {const method=command.data.geoVerification?'geo_punctual':'web';const result=await recordEvent({employeeId,method,userId:access.actor.id},command.data.eventType,command.data.deviceOccurredAt,assertIdempotencyKey(request.headers.get('idempotency-key')),cid,command.data.geoVerification);if(!result.replayed)void processCalculationOutbox().catch(()=>undefined);return response(result,cid,result.replayed?200:201);}catch(e){return failure(e,cid);}}
