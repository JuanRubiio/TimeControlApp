...
export async function POST(request:NextRequest) {const cid=correlationId();const access=await actorFor(request,'time-event.create:self',cid);if('response'in access)return access.response;const command=timeEventCommand.safeParse(await request.json().catch(()=>null));if(!command.success)return error('VALIDATION_FAILED','La solicitud de fichaje no es válida.',422,cid);const employeeId=await employeeForUser(access.actor.id);if(!employeeId)return error('FORBIDDEN','No existe una ficha de empleado activa para fichar.',403,cid);try {const result=await recordEvent({employeeId,method:'web',userId:access.actor.id},command.data.eventType,command.data.deviceOccurredAt,assertIdempotencyKey(request.headers.get('idempotency-key')),cid);if(!result.replayed)void processCalculationOutbox().catch(()=>undefined);return response(result,cid,result.replayed?200:201);}catch(e){return failure(e,cid);}}
