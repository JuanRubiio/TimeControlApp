...
- Solicita: empleado con `correction.create:self`, exclusivamente su ficha. Lee: propio con `correction.read:self`; responsable con `correction.read:scope` y centro asignado; Administración sólo consulta la trazabilidad autorizada. Decide: exclusivamente el responsable con `correction.decide` y ámbito de centro. No existe excepción administrativa. El motivo es obligatorio al solicitar y rechazar.
