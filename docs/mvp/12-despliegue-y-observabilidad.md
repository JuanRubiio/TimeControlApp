...
### Evidencia operativa S15 posterior — 13/09/2026

S15 ejecutó la cadena completa con tres PostgreSQL sintéticos aislados: fuente migrada/seeded, destino con el mismo `ENVIRONMENT_ID` y destino cruzado. El backup cifrado se restauró en el destino válido, con checksum, UUID y una exportación expirada verificados; el UUID cruzado se rechazó en el descifrado autenticado antes de `pg_restore` y no creó tablas en el destino. La prueba detectó `pg_dump` 15 contra servidor 16, un pipeline que podía publicar artefacto vacío y una comprobación checksum desde el directorio incorrecto. S15 corrigió `Dockerfile.ops`, backup y restore; se añaden pruebas estáticas de versión, limpieza y orden de validación. Esta evidencia no aprueba proveedor, RPO/RTO, agenda de copias, TLS/WAF ni operación con datos reales.

La imagen de aplicación también se reconstruyó correctamente con Node 20.19.0 y ejecutó 113/113 pruebas dentro del contenedor. Durante la validación se corrigió que el runner, que incluye la suite, no incluía `Dockerfile.ops` usado por una prueba de imagen operativa. El build conserva cinco avisos de Turbopack ya conocidos: cuatro de acceso dinámico al almacenamiento de exportaciones y uno de trazado Edge de `node:crypto` desde instrumentación; no impiden construir, pero requieren revisión propietaria de S9 y S5/S19 antes de un despliegue gestionado.
