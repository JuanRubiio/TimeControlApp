...
### Corrección de destino posterior al login — 12/09/2026

La observación visual del piloto detectó que, tras cerrar una sesión de empleado, el acceso de responsable heredaba el destino seguro fijo `/employee`. Se corrige sin ampliar privilegios: `passwordLogin` y el cierre MFA devuelven un destino interno calculado con los roles vigentes; el cliente prioriza cualquier `returnTo` que supere la lista blanca. Administración, responsable y auditor llegan a `/admin`; empleado llega a `/employee`. La ruta raíz aplica el mismo criterio y `/employee` redirige a una persona sin rol de empleado a su inicio administrativo. Las páginas y APIs siguen siendo la autoridad de autorización; este destino no concede permisos.
