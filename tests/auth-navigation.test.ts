...
describe('S19 navegación protegida', () => {
  it.each(['/employee', '/employee/history/2026-09-12', '/manager', '/admin', '/admin/corrections/a'])('conserva sólo rutas privadas internas permitidas: %s', (value) => expect(safeReturnTo(value)).toBe(value));
  ...
  it.each([[['employee'], '/employee'], [['manager'], '/manager'], [['admin'], '/admin'], [['auditor'], '/admin'], [[], '/employee']] as const)('elige el inicio correcto para el rol %j', (roles, destination) => expect(defaultDestination({ roles })).toBe(destination));
});
